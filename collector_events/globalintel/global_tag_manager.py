...
class GlobalTagManager(Loggable):
    ...
    def __init__(
        ...
        Args:
            ...
            llm_enrich: When True, call LLM AI Trend Oracle to refine bias and
                risk_score before persisting the tag. Requires DEEPSEEK_API_KEY
    ...
    async def _create_tag(self, item: IntelItem) -> GlobalTag | None:
        ...
        # ── Stage 3: LLM enrichment via AI Trend Oracle ──────────────
        if self._llm_enrich:
            enriched = await self._llm_enrich_tag(item, asset, bias, risk_score)
            if enriched:
    ...
    # ── AI Trend Oracle (Stage 3 LLM enrichment) ─────────────────────────

    async def _llm_enrich_tag(
        self,
        item: IntelItem,
        asset: str,
        current_bias: str,
        ...
        """Call LLM API to refine bias direction and risk_score for a GlobalTag.

        Uses DeepSeek (DEEPSEEK_API_KEY) as primary provider; falls back to
        Claude (CLAUDE_API_KEY) if DeepSeek key is absent.

        Returns a dict with keys ``bias``, ``risk_score``, ``reasoning`` or
        ``None`` if no API key is available or the call fails.

        Args:
            item: The IntelItem that triggered tag creation.
            asset: The forex pair being tagged (e.g., "EURUSD").
            current_bias: Rule-based bias derived from domain/severity.
            current_risk: Rule-based risk_score (0.0 – 1.0).

        Returns:
            dict with refined ``bias``, ``risk_score``, and ``reasoning``;
        ...
        if not api_key:
            ...
            provider = "claude"
        if not api_key:
            self.log.debug("GlobalTagManager: no LLM API key — skipping enrichment")
            return None
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                if provider == "deepseek":
                    result = await _call_deepseek(session, api_key, prompt)
                else:
                    result = await _call_claude(session, api_key, prompt)
            ...
            self.log.warning(
                "GlobalTagManager: LLM enrichment failed for %s — %s", asset, exc
            )
            return None
...
async def _call_deepseek(
    ...
    async with session.post(_DEEPSEEK_CHAT_URL, headers=headers, json=body) as resp:
...
async def _call_claude(
    ...
    async with session.post(_CLAUDE_MESSAGES_URL, headers=headers, json=body) as resp:
        resp.raise_for_status()
        data = await resp.json()
...
def _parse_oracle_response(raw: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        if bias not in IntelBias.ALL:
            return None   # Invalid bias — discard enrichment
